...
enum CourseInvitesEndpoint {
    ...
    case create(courseId: String, request: CreateInviteRequestDTO, baseURL: URL)
    ...
    func makeURLRequest() throws -> URLRequest {
        ...
        case let .list(courseId, baseURL):
            ...
            var request = URLRequest(
                url: baseURL.appendingPathComponent("\(basePath)/\(courseId)/invites")
            )
        ...
        case let .revoke(courseId, inviteId, baseURL):
            ...
            var request = URLRequest(
                url: baseURL.appendingPathComponent("\(basePath)/\(courseId)/invites/\(inviteId)")
            )
